...
We define global exclusion keywords which we use to exclude certain codes from any of the outcome-specific dictionaries. These codes are related to the absence of a condition, screening for a condition, normal result for a condition, family history of a condition, various administrative statuses and more, and are not wanted when we are trying to capture the codes that are specific to certain outcomes.
```{r}
