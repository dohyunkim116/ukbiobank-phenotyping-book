...
tte_dat_list_per_outcome_eur <-
  tte_dat_list_per_outcome %>% lapply(function(tte_dat_list){
    lapply(tte_dat_list,function(tte_dat){
        tte_dat %>% filter(EUR == 1) %>% mutate(across(.cols = starts_with("cv"),.fns = scale)) %>% 
        mutate(across(.cols = starts_with(c("MET_hours","avg_uacr","cv","PGS")),.fns = scale))
    })
})
...
tte_dat_list_per_outcome_eur_glycemic_drug1 <-
  tte_dat_list_per_outcome %>% lapply(function(tte_dat_list){
    lapply(tte_dat_list,function(tte_dat){
        tte_dat %>% filter(EUR == 1, self_insulin == 1) %>% mutate(across(.cols = starts_with("cv"),.fns = scale)) %>% 
        mutate(across(.cols = starts_with(c("MET_hours","avg_uacr","cv","PGS")),.fns = scale))
    })
})
...
tte_dat_list_per_outcome_eur_glycemic_drug0 <-
  tte_dat_list_per_outcome %>% lapply(function(tte_dat_list){
    lapply(tte_dat_list,function(tte_dat){
        tte_dat %>% filter(EUR == 1, self_insulin == 0) %>% mutate(across(.cols = starts_with("cv"),.fns = scale)) %>% 
        mutate(across(.cols = starts_with(c("MET_hours","avg_uacr","cv","PGS")),.fns = scale))
    })
})
```
...
pa_preds <- "MET_hours"
scaled_pa_preds <- "scale(MET_hours)"
...
save(list = names(res_list),file = "results/res_primary_analysis_step_dm2.RData")
save.image(file = "results/session_res_primary_ana_dm2.RData")
```
