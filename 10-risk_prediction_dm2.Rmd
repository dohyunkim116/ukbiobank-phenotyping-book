---
title: "Complication risk prediction. DM2."
author: "Do Hyun Kim"
...
tte_dat_list_per_outcome <- 
  lapply(tte_datasets,function(tte){
    lapply(imputed_baseline_dat_list,function(baseline_dat){
      ...
      merged_dat %>% filter(EUR == 1) %>% 
        left_join(dm_type_tab,by="f.eid") %>% 
        filter(dm_type == "Type2") %>% 
        select(-c(EUR,dm_type)) %>% na.omit()
    })
  })
```
...
saveRDS(res,"results/res_risk_prediction_main_training_dm2.RDS")
```

Create input data frame for AUC bar plot generation
```{r}
res <- readRDS("results/res_risk_prediction_main_training_dm2.RDS")
...
```{r}
pdf("figures/auc_bar_plot_across_outcomes_dm2.pdf")
print(auc_bar_plot)
dev.off()
```

```{r}
png("figures/auc_bar_plot_across_outcomes_dm2.png")
print(auc_bar_plot)
dev.off()
```
...
```{r}
pdf("figures/km_plot_main_dm2.pdf")
print(km_main)
dev.off()
```

```{r}
png("figures/km_plot_main_dm2.png")
print(km_main)
dev.off()
```
...
lapply(outcome_names,function(outcome_name){
  ...
  fit <- survfit(Surv(time_to_event,event)~risk,data = dat)
  file_name <- paste0("figures/km_plot_",outcome_name,"_dm2.png")
  temp <- survminer::ggsurvplot(fit,data=df_long_km,pval=T,scales = "free",ggtheme = theme_minimal(),legend = "right",
                                conf.int = T) + 
    xlab(label = "Time(years)")
  png(file_name)
  print(temp)
  ...
  file_name2 <- paste0("figures/km_plot_",outcome_name,"_dm2.pdf")
  pdf(file_name2)
  print(temp)
  dev.off()
})
...
flextable::save_as_docx(values = list(var_sel_sum_tab_ft), pr_section = sect_properties, path = "tables/tableS7_dm2.docx")
```
