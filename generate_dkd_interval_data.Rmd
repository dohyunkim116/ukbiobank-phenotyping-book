...
#Remove subjects with a time-to-event upper bound less than 5 years
tab <- tab %>% filter(upper>5)
...
tab <- tab %>% mutate(lower = ifelse(is.na(known_comp) | known_comp, upper, ifelse(is.na(event_dt_egfr),0, decimal_date(event_dt_comp)-decimal_date(event_dt_egfr))))

#Remove subjects with lower bounds less than 5 years to create a greater likelihood that the complication is diabetes related
tab <- tab %>% filter(lower>5 | is.na(lower))
...
```{r}
saveRDS(dr_interval, 'generated_data/dr_interval.RDS')
```
