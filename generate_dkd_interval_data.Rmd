...
tab <- tab %>% filter(event == 0 | (event == 1 & nonsense_case == 0 & prior_comp == 0 & known_dm==1)) %>% filter(event == 1 | (event == 0 & nonsense_ctrl == 0 & ctrl_exclude == 0 & ctrl_include == 1 & fut_less_than_5yrs == 0))
...
tab <- tab %>% mutate(lower = ifelse(known_comp, upper, ifelse(is.na(event_dt_egfr),0, decimal_date(event_dt_comp)-decimal_date(event_dt_egfr))))
