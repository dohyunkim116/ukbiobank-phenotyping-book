...
#currently 1 (which translates to 1 year)
 but may be changed
margin <- 1
...
table(dm_date_known$known, useNA='always')
```
...
Load diagnosis dates for DM with flag indicating whether or not they are known, dates of first occurence of cvd, and demographic information 
```{r}
...
#These patients are excluded to allow a greater likelihood that remaining patients have diabetes-related cvd
...
tab <- tab %>% filter(event == 0 | (event == 1 & nonsense_case == 0 & prior_comp == 0)) %>% filter(event == 1 | (event == 0 & nonsense_ctrl == 0 & ctrl_exclude == 0 & fut_less_than_5yrs == 0))
